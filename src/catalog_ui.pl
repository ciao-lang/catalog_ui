...
setup :-
    ...
    set_layout(bootstrap_layout(default, singletab(catalog_ui))).
...
all_bundles_html -->
    ...
    %
    [h2([env(a, [href='/'], [image('/images/ciao-logo.png', [height='40px'])]), ' Bundle Catalog'])],
    ['This is the catalog of available Ciao bundles. ',
     'The links below provide detailed information for each bundle ',
...
bundle_desc(M) -->
    ( { guess_bundle_title(M, Title) } -> [Title] ; [] ).
...
gen_build_status(_Bundle) -->
    ...
    ['Status and logs: '],
    [env(a, [href='https://travis-ci.org/ciao-lang/ciao'], ['Linux and macOS'])], [' | '],
    [env(a, [href='https://ci.appveyor.com/project/jfmc/ciao'], ['Windows'])], [\\],
    ['Binaries: '],
    % ['Archived prebuilt versions: '],
    [env(a, [href='https://dl.bintray.com/ciao-lang/builds/'], ['here'])].

% ---------------------------------------------------------------------------
% Bundle dependencies

has_deps(Bundle) :-
    ( bundle_deps(Bundle, _)
    ; show_revdeps, bundle_revdeps(Bundle, _)
    ),
    !.

gen_deps_refs(Bundle) -->
    { findall(R, bundle_deps(Bundle, R), Deps) },
    ( { Deps = [] } -> []
    ; ['Dependencies:'],
      bundle_links(Deps), [\\]
    ).
...
gen_revdeps_refs(Bundle) -->
    { findall(R, bundle_revdeps(Bundle, R), RevDeps) },
    ( { RevDeps = [] } -> []
    ; ['Used by:'],
...
has_manual_entries(Bundle) :- bundle_manuals(Bundle, _, _), !.
...
bundle_manual_link(Bundle, Name, HRef) :-
    bundle_manuals(Bundle, Name, HRef0),
